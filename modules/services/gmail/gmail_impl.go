...
func (srv *GmailSrvImpl) Created(ctx context.Context, gmailIn models.Gmail) (err error) {
	...
	gmailIn.Message = fmt.Sprintf("happy birthday to our beloved customer %v, today especially for you we are providing a special birthday promo code with the promo code %v  with a 20 percent discount for all product ", gmailIn.Name, gmailIn.Promo)
